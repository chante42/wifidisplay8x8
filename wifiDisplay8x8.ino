...
WiFiUDP ntpUDP;
// 'time.nist.gov' is used with 60 seconds update interval and
// offset de 2 heures -=- Comment gérée le changement d'heure ???????
...
void setup () {
  ...
  strncat(message, " ", MAXLENMESSAGE - 1);
  strncat(message, timeClient.getFormattedTime().c_str(),MAXLENMESSAGE - 1);
